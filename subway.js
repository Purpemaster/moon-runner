...
var Train = {};
var TrainBB = {};
var Dog; // (wurde im Original benutzt, hier explizit deklariert)
...
function init() {
    ...
    // ===== Responsive Renderer (mobile friendly) =====
    ...
    // ===== Mobile Swipe Controls (SS-like) =====
    const canvasEl = renderer.domElement;
    let touchStartX = 0, touchStartY = 0, touching = false;

    function laneLeft() {
        // eine Spur nach LINKS (X +)
        if (!Surfer) return;
        Surfer.position.x = Math.min(2, Surfer.position.x + 1.3);
        camera.rotation.y -= player.turnSpeed;
    }
    function laneRight() {
        // eine Spur nach RECHTS (X -)
        if (!Surfer) return;
        Surfer.position.x = Math.max(-2.5, Surfer.position.x - 1.3);
        camera.rotation.y += player.turnSpeed;
    }
    function doJump() {
        if (!Surfer) return;
        if (Surfer.position.y <= 0.01) {
            Jump = 1;
    ...
    canvasEl.addEventListener('touchend', function (e) {
        ...
        if (Math.abs(dx) > Math.abs(dy)) {
            if (dx > 30) laneRight();   // Swipe →
            if (dx < -30) laneLeft();   // Swipe ←
        } else {
            if (dy < -30) doJump();     // Swipe ↑
            if (dy >  30) doSlide();    // Swipe ↓
        }
    }, { passive: true });
